class World6 extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.image(400, 300, 'mapbasenuji').setOrigin(0.5, 0.5).setDisplaySize(800, 600);

        const mapLabel = this.add.image(400, 300, 'maplablenuji').setOrigin(0.5, 0.5).setAlpha(0);
        ...
        const hotspots = [
            ...
            {
                x: 250, y: 320, width: 80, height: 80,
                level: "l17",
                text2: "Level 2: City of the Silversmiths"
            },
            {
                x: 420, y: 460, width: 80, height: 80,
                level: "l18",
    ...
    hotspots.forEach(hotspot => {
        this.add.rectangle(hotspot.x, hotspot.y, hotspot.width, hotspot.height, 0x000000, 0)
            ...
            .on('pointerdown', () => {
                arrow.setVisible(false);
                this.sound.play('ui');
                this.scene.get('OSTManager').crossfadeTo('world6', 300);
                goToLevel("World 6: Kingdom of Nuji", hotspot.text2, "SELECTED!", hotspot.level);
            });
    });
